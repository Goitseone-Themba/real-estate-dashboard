...
var V = (e) => {
  let { devtoolsUrl: s2 } = import_react4.default.useContext(O), [m, n] = import_react4.default.useState([]), c = import_react4.default.useCallback(async () => (await (await fetch(`${s2 ?? "http://localhost:5001"}/api/unique-trace-items`)).json()).data, [s2]), o2 = import_react4.default.useCallback((r, p2) => {
    let t = r, h, i2, v = false;
    for (; !v && t; )
      h = o(t), i2 = s(h), v = p2.includes(i(h) ?? ""), v || (t = l(t));
    ...
    if (i2.nameFiber && i2.stateNode) {
      let x = p(i2.stateNode), P = i(i2.nameFiber);
      x && P && t.push({ element: x, name: P });
    }
    for (let x = 0; x < ((v = r == null ? void 0 : r.children) == null ? void 0 : v.length); x++)
      t.push(...d2(r.children[x], p2));
    return t;
...
var E = ({ children: e }) => (import_react5.default.useEffect(() => {
...
var oe = ({ element: e, name: s2, onSelect: m }) => {
  ...
  import_react6.default.useEffect(() => {
    let l2 = debounce_default(() => {
      var P, N, I, L2, R;
      let { top: a, left: r, width: p2, height: t } = e.getBoundingClientRect(), { scrollLeft: h, scrollTop: i2 } = document.documentElement, v = r + h, x = a + i2;
      (P = c.current) == null || P.style.setProperty("left", `${v}px`), (N = c.current) == null || N.style.setProperty("top", `${x}px`), (I = c.current) == null || I.style.setProperty("width", `${p2}px`), (L2 = c.current) == null || L2.style.setProperty("height", `${t}px`), (R = c.current) == null || R.style.setProperty("opacity", "1");
...
var k = ({ elements: e, onSelect: s2 }) => {
  ...
  return import_react6.default.useEffect(() => {
    ...
    return () => 0;
  }, []), m ? import_react6.default.createElement(import_react6.default.Fragment, null, (0, import_react_dom.createPortal)(e.map((c, o2) => import_react6.default.createElement(oe, { key: `selector-element-${o2}-${c.name}`, ...c, onSelect: s2 })), m), import_react6.default.createElement(E, null, `
          .selector-xray-box {
            display: flex;
            margin: 0;
            padding: 0;
...
var U = ({ active: e, setActive: s2, onHighlight: m, icon: n, style: c }) => {
  let { selectableElements: o2 } = V(e);
  ...
  } }, n), e && import_react3.default.createElement(k, { elements: o2, onSelect: (d2) => {
    m(d2), s2(false);
  } }), import_react3.default.createElement(E, null, `
...
var G = ({ onClick: e, onSelectorHighlight: s2, selectorActive: m, setSelectorActive: n }) => import_react2.default.createElement("div", { role: "button", className: "devtools-selector-pin-box", onClick: e }, import_react2.default.createElement($, null), import_react2.default.createElement(U, { style: { position: "absolute", top: 5, right: 18, width: "16px", height: "16px" }, icon: import_react2.default.createElement(z, { width: 16, height: 16, style: { pointerEvents: "none" } }), onHighlight: s2, active: m, setActive: n }), import_react2.default.createElement(E, null, `
...
var ne = () => typeof window < "u" ? window.innerWidth * 0.7 : 1007.9999999999999;
var re = () => typeof window < "u" ? window.innerHeight * 0.7 : 630;
var C = (e) => {
  switch (e) {
    case "left":
...
var A = (e, s2) => {
  let m = { width: ne(), height: re() }, n = T2(e), c = B(e), o2 = Math.min(n, (s2 ?? m).width), l2 = Math.min(c, (s2 ?? m).height);
...
var Y = ({ placement: e, visible: s2, children: m }) => {
  let [n, c] = import_react8.default.useState(false), [o2, l2] = import_react8.default.useState(null), [d2, a] = import_react8.default.useState(null), [r, p2] = import_react8.default.useState(() => {
    let t = A(e);
    return { width: b(t.width), height: b(t.height) };
  });
  return import_react8.default.useEffect(() => {
    let t = () => {
      p2((h) => {
        let i2 = A(e, h);
        return { width: b(i2.width), height: b(i2.height) };
      });
    };
    ...
    let t = (h) => {
      if ((o2 == null ? void 0 : o2[1]) === "x") {
        let i2 = h.clientX - ((d2 == null ? void 0 : d2.x) ?? h.clientX), v = r.width + (o2 === "lx" ? -i2 : i2) * 2;
        p2((x) => ({ ...x, width: b(Math.min(T2(e), Math.max(640, v))) }));
      } else if ((o2 == null ? void 0 : o2[1]) === "y") {
        let i2 = h.clientY - ((d2 == null ? void 0 : d2.y) ?? h.clientY), v = r.height + (o2 === "ty" ? -i2 : i2) * 1;
        p2((x) => ({ ...x, height: b(Math.min(B(e), Math.max(360, v))) }));
      }
    };
    if (o2 !== null)
  ...
  } }, import_react8.default.createElement(H, null)), import_react8.default.createElement("div", { style: { position: "absolute", right: 0, top: "50%", width: "10px", height: "26px", transform: "translateY(-13px) translateX(5px)", cursor: "col-resize", transition: "opacity ease-in-out 0.2s", pointerEvents: n || o2 ? "auto" : "none", opacity: n || o2 ? 1 : 0 }, onMouseDown: (t) => {
    l2("rx"), a({ x: t.clientX, y: t.clientY }), t.preventDefault();
  } }, import_react8.default.createElement(H, null)), import_react8.default.createElement("div", { style: { position: "absolute", left: "50%", top: 0, width: "26px", height: "10px", transform: "translateY(-5px) translateX(-13px)", cursor: "row-resize", transition: "opacity ease-in-out 0.2s", pointerEvents: n || o2 ? "auto" : "none", opacity: n || o2 ? 1 : 0 }, onMouseDown: (t) => {
...
var de = false ? () => null : () => {
  let [e, s2] = import_react.default.useState(false), [m, n] = import_react.default.useState(false), [c] = import_react.default.useState("bottom"), { devtoolsUrl: o2, ws: l2 } = import_react.default.useContext(O), [d2, a] = import_react.default.useState(0), [r, p2] = import_react.default.useState(false), t = import_react.default.useCallback((i2) => {
    l2 && d(l2, T.DEVTOOLS_HIGHLIGHT_IN_MONITOR, { name: i2 }), n(true);
  ...
  }, [e]), e ? import_react.default.createElement("div", { style: { position: "fixed", left: `${Math.round(d2 / 2)}px`, transform: "translateX(-50%)", bottom: 0, zIndex: 99999 } }, import_react.default.createElement(G, { onClick: () => {
    n((i2) => !i2), p2(false);
  }, onSelectorHighlight: t, selectorActive: r, setSelectorActive: p2 }), import_react.default.createElement(Y, { visible: m, placement: c }, ({ resizing: i2 }) => import_react.default.createElement("iframe", { allow: "clipboard-write;", src: o2, srcDoc: o2 ? void 0 : `
...
var ue = false ? ({ children: e }) => e : ({ children: e }) => import_react10.default.createElement(L, null, e);
export {
  de as DevtoolsPanel,
  ue as DevtoolsProvider
};
